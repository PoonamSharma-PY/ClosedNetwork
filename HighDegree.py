...
# OUT_CSV = "HighDeg_PMCSN.csv"
# OUT_JSON = "HighDeg_PMCSN.json"
...
# def select_topk_high_outdeg(G, k):
#     """
#     Top-k by out-degree. Tie-breaker by node id ascending for determinism.
#     If k exceeds node count, returns all nodes.
#     """
#     od = list(G.out_degree())
#     od.sort(key=lambda x: (-x[1], x[0]))
#     return [u for u, _ in od[:k]]

# # --- Algorithm ---
# def run_highdeg_pmcsn(G, k, l, costs, benefits):
#     start = time.perf_counter()

#     # 1) Seeds = top-k by out-degree (on the full graph)
#     seed_set = select_topk_high_outdeg(G, k)
...
#         row = run_highdeg_pmcsn(G, k, l, costs, benefits)
#         results.append(row)
