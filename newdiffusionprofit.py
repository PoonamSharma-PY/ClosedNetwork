...
# # --- Configuration ---
# INPUT_FOLDER = "Closed_Network_Outputs_l8_k2_x1000"
# SEED_FILE = os.path.join(INPUT_FOLDER, "seed_sets.json")
# COST_FILE = "cost.txt"
# BENEFIT_FILE = "benefit.txt"
# SIMULATIONS = 10000
# OUTPUT_JSON = "closed_network_profits.json"
# OUTPUT_CSV = "closed_network_profits.csv"
...
#     }
#     all_graph_profits.append(result)

#     if profit > best_profit:
#         best_profit = profit
#         best_graph_info = result

# # --- Save All Results ---
# with open(OUTPUT_JSON, "w") as f_json:
#     json.dump(all_graph_profits, f_json, indent=2)

# df = pd.DataFrame(all_graph_profits)
...
# # ---------------- Configuration ----------------
# ROOT_DIR = "."
# FOLDER_GLOB = os.path.join(ROOT_DIR, "Closed_Network_Outputs_l*_k*_x*")  # folder pattern

# COST_FILE = "cost.txt"
# BENEFIT_FILE = "benefit.txt"
# SIMULATIONS = 10000  # Number of diffusion simulations per closed network (per graph)

# BEST_PER_KL_JSON = "best_closed_network_per_k_l.json"
# BEST_PER_KL_CSV  = "best_closed_network_per_k_l.csv"
...
# def parse_folder_kl(folder_name):
#     """
#     Parses l, k, x from folder names like: Closed_Network_Outputs_l8_k2_x1000
#     Returns (l_val, k_val, x_val)
#     """
#     m = re.search(r"_l(\d+)_k(\d+)_x(\d+)", folder_name)
#     if not m:
#         raise ValueError(f"Cannot parse k/l/x from folder name: {folder_name}")
...
#     for p in (BEST_PER_KL_CSV, PROGRESS_ALL_GRAPHS_CSV, BEST_PER_KL_JSON):
#         if os.path.exists(p):
#             os.remove(p)

#     winners = []           # one best row per (k,l)
#     global_best = None     # best across all (k,l)

#     # Process each (k,l) folder
#     for folder in tqdm(folders, desc="Folders"):
#         folder_name = os.path.basename(folder.rstrip("/\\"))
#         try:
#             l_val, k_val, x_val = parse_folder_kl(folder_name)  # x_val == No. of Samples per (k,l)
#         except ValueError:
#             continue
...
#         best_row = None  # winner for this (k,l)
#         progress_buffer = []  # buffer to append progress in chunks

#         # --- Measure total time to find best for this (k,l) ---
...
#                 "No. of Samples": int(x_val),           # 🔹 samples per (k,l)
#                 "Value of l": int(l_val),
#                 "Value of k": int(k_val),
#                 "Graph Id": graph_id,
...
#         # Total time to find the best for this (k,l)
#         folder_exec_time = time.perf_counter() - folder_t0

#         # Save/update best-per-(k,l) CSV immediately after finishing folder
#         if best_row is not None:
#             # Overwrite the timing field to be the TOTAL time for the (k,l) search
...
#             append_rows_csv(BEST_PER_KL_CSV, [best_row_out])

#             print(f"\n🏁 Winner for (k={best_row_out['Value of k']}, l={best_row_out['Value of l']}): "
...
#     with open(BEST_PER_KL_JSON, "w") as f:
#         json.dump(winners, f, indent=2)

#     # Print global best (by profit)
#     if global_best:
#         print("\n🌟 Global Best Across All (k,l):")
#         for k, v in global_best.items():
#             print(f"{k}: {v}")
#         print(f"\n✅ Best-per-(k,l): {BEST_PER_KL_CSV}")
